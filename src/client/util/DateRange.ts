...
export class DateRange {
    ...
    private static BEFORE_TIMES = new Date(1900,1);
    private static AFTER_TIMES = new Date(2300,1);
    ...
    includes (date:Date): boolean {
        return DateRange.toEpoch(date) >= DateRange.toEpoch(this._start) &&
            DateRange.toEpoch(date) <= DateRange.toEpoch(this._end);
    }

    equals (otherRange: DateRange): boolean {
        return DateRange.toEpoch(otherRange.start) === DateRange.toEpoch(this._start) &&
            DateRange.toEpoch(otherRange.end) === DateRange.toEpoch(this._end);
    }

    overlaps (otherRange: DateRange): boolean {
        return this.includesRange(otherRange) || this.includes(otherRange.start) || this.includes(otherRange.end)
    }
